...
const Slider: React.FC = () => {
  ...
  useEffect(() => {
    const timeToDeleteCaracter = 35;
    const delayToWritten = 200;

    writtenTitle.split('').forEach((l, i) => {
      setTimeout(() => {
        setWrittenTitle((old) => old.substring(0, old.length - 1))
      }, i * timeToDeleteCaracter)
    })

    setTimeout(() => {
      title.split('').forEach((caracter, i) => {
        setTimeout(() => {
          setWrittenTitle((old) => old+caracter)

          if(i + 1 === title.length){
            setAnimationDescription('in')
          }
        }, i * 65)
      })
  ...
  return (
    <S.Wrapper>
      <S.Header>{id < 10 ? `0${id}` : id}<S.BarProgress animation={animationDescription} /></S.Header>
  
      <S.Title>{writtenTitle}</S.Title>
  
      <S.Description animation={animationDescription}>{description}</S.Description>
